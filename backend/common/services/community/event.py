...
import grpc
from backend.common.proto import community_event_pb2, community_event_pb2_grpc


class CommunityEventClient:
    """
    ...
    def __new__(cls):
        with cls._lock:
            if cls._instance is None:
                cls._instance = super(CommunityEventClient, cls).__new__(cls)
    ...
    def initialise(cls, endpoint_url: str, redis_url: str):
        """
        Set the configuration for CommunityEventClient. This would also handle auth/encryption.
    ...
    def _initialise(self):
        ...
        """
        if not self._config:
            raise ValueError("CommunityEventClient not initialised")
    ...
    def _reconnect(self):
        ...
        self._stub = community_event_pb2_grpc.CommunityEventStub(self._channel)
        self._redis_client = redis.from_url(self._redis_url)

    def create(self, req: community_event_pb2.EventDataRequest, session_id: int) -> community_event_pb2.CreateResponse:
        """
        Create a new community event in the Community service.
        """
        session_keys = self._redis_client.keys(f"sid:{session_id}:*")
        if not session_keys:
            return community_event_pb2.CreateResponse(
        ...
        if user_data:
            ...
            return self._grpc_call_with_retry(self._stub.CreateEvent, req)
    ...
    def update(self, req: community_event_pb2.EditEventRequest, session_id) -> community_event_pb2.EventResponse:
        """
        Updates a selected community in the Community service.
        """
        session_keys = self._redis_client.keys(f"sid:{session_id}:*")
        if not session_keys:
            return community_event_pb2.EventResponse(
                success=False,
                http_status=401,
    ...
    def view(self, req: community_event_pb2.ViewEvent, session_id) -> community_event_pb2.ViewResponse:
        """
        Views all of a selected community event in the Community service.
        """
        session_keys = self._redis_client.keys(f"sid:{session_id}:*")
        if not session_keys:
            return community_event_pb2.ViewResponse(
                status=community_event_pb2.EventResponse(
                    success=False,
                    http_status=401,
                    error_message=['Action Requires User To Be Logged In']
        ...
        if user_data:
            ...
            return self._grpc_call_with_retry(self._stub.ViewEvents, req)
    ...
    def view_one(self, req: community_event_pb2.ViewOneRequest, session_id) -> community_event_pb2.ViewResponse:
        """
        Views a selected community event in the Community service.
        """
        ...
        if user_data:
            ...
            return self._grpc_call_with_retry(self._stub.ViewOneEvent, req)
    ...
    def view_global(self, req: community_event_pb2.ViewGlobalRequest) -> community_event_pb2.ViewResponse:
        """
        Views all public community event in the Community service.
        """
        return self._grpc_call_with_retry(self._stub.ViewGlobalEvents, req)
    
    def delete(self, req: community_event_pb2.DeleteEventRequest, session_id) -> community_event_pb2.EventResponse:
        """
        Deletes a community event in the Community service.
        """
        session_keys = self._redis_client.keys(f"sid:{session_id}:*")
        if not session_keys:
            return community_event_pb2.EventResponse(
        ...
        if user_data:
            ...
            return self._grpc_call_with_retry(self._stub.DeleteEvent, req)
