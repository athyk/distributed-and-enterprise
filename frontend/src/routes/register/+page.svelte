<script lang="ts">
	...
	async function register(){
		...
		const dobString = ("0" + dobDate.getDate()).slice(-2) + "-" + ("0" + (dobDate.getMonth() + 1)).slice(-2) + "-" + dobDate.getFullYear();
		...
		<svelte:fragment slot="pages">
			{#if step === 1}
				<Page
					bind:email
					bind:password
					password_comfirm={true}
					bind:password_comfirm_value={password_comfirm_value}
				/>
			{/if}
			{#if step === 2}
				<Page
