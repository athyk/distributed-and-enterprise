<script lang="ts">
  ...
  <div class="flex min-h-screen bg-gray-100 dark:bg-gray-900">
    ...
    <aside class="w-64 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 p-4">
      ...
      <nav class="space-y-2">
        <button on:click={() => (activeTab = "update")} class="w-full text-left px-3 py-2 rounded-md 
          hover:bg-gray-200 dark:hover:bg-gray-700 transition focus:outline-none 
          text-gray-800 dark:text-gray-200 {activeTab === 'update' ? 'bg-gray-200 dark:bg-gray-700 font-semibold' : ''}">
          Update Community
        </button>
        <button on:click={() => (activeTab = "delete")} class="w-full text-left px-3 py-2 rounded-md 
          hover:bg-gray-200 dark:hover:bg-gray-700 transition focus:outline-none 
          text-gray-800 dark:text-gray-200 {activeTab === 'delete' ? 'bg-gray-200 dark:bg-gray-700 font-semibold' : ''}">
          Delete Community
        </button>
      </nav>
    </aside>
    ...
    <main class="flex-1 p-8">
      {#if activeTab === "update"}
        ...
        <div class="bg-white dark:bg-gray-800 rounded-md p-4 shadow">
          ...
          <input id="name" type="text" bind:value={formData.name} class="block w-full mt-1 mb-3 rounded-md border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-gray-200" />
  
          <!-- Description -->
          <label for="description" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Description</label>
          <textarea id="description" bind:value={formData.description} class="block w-full mt-1 mb-3 rounded-md border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-gray-200"></textarea>
