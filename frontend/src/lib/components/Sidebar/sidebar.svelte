<script lang="ts">
	...
	let expanded: Record<
		'categories' | 'communityPub' | 'communityTags' | 'communityDegree' | 'price',
		boolean
	> = {
		categories: true,
		communityPub: true,
...
<div class="hidden w-72 bg-gray-900 p-4 text-white shadow-lg lg:block">
	<!-- Categories -->
	<div class="border-b border-gray-700 py-2">
		<div
			class="flex cursor-pointer items-center justify-between p-2 hover:text-yellow-400"
		...
		{#if expanded.categories}
			{#each $categories as category}
				<label class="flex cursor-pointer items-center gap-2 p-2 hover:bg-gray-800">
					<input
						type="checkbox"
						class="accent-yellow-400"
						on:change={() => toggleFilter(selectedCategories, category.name)}
					/>
	...
	<div class="border-b border-gray-700 py-2">
		<div
			class="flex cursor-pointer items-center justify-between p-2 hover:text-yellow-400"
			on:click={() => toggleSection('communityPub')}
		>
			Community Type <span>{expanded.communityPub ? '▲' : '▼'}</span>
		</div>
		{#if expanded.communityPub}
			{#each $communityPub as pub}
				<label class="flex cursor-pointer items-center gap-2 rounded p-2 hover:bg-gray-800">
					<input
						type="checkbox"
						class="accent-yellow-400"
						on:change={() => toggleFilter(selectedCommunityPub, pub.name)}
					/>
	...
	<div class="border-b border-gray-700 py-2">
		<div
			class="flex cursor-pointer items-center justify-between p-2 hover:text-yellow-400"
			on:click={() => toggleSection('communityTags')}
		>
			...
			{#each $communityTags as tag}
				<label class="flex cursor-pointer items-center gap-2 rounded p-2 hover:bg-gray-800">
					<input
						type="checkbox"
						class="accent-yellow-400"
						on:change={() => toggleFilter(selectedCommunityTags, tag.name)}
					/>
	...
	<div class="border-b border-gray-700 py-2">
		<div
			class="flex cursor-pointer items-center justify-between p-2 hover:text-yellow-400"
			on:click={() => toggleSection('communityDegree')}
		>
			...
			{#each $communityDegree as degree}
				<label class="flex cursor-pointer items-center gap-2 rounded p-2 hover:bg-gray-800">
					<input
						type="checkbox"
						class="accent-yellow-400"
						on:change={() => toggleFilter(selectedCommunityDegree, degree.name)}
					/>
	...
	<button
		class="mt-4 w-full rounded-lg bg-red-600 py-2 text-white transition duration-300 hover:bg-red-700"
